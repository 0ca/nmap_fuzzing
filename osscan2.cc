...
HostOsScan::HostOsScan(Target *t) {
  ...
  tcpPortBase = 0;//o.magic_port_set? o.magic_port : o.magic_port;// + get_random_u8();
  udpPortBase = 0;//o.magic_port_set? o.magic_port : o.magic_port;// + get_random_u8();
  reInitScanSystem();
...
void HostOsScan::reInitScanSystem() {
  tcpSeqBase = 0;//get_random_u32();
  tcpAck = 0;//get_random_u32();
  tcpMss = 265;
  icmpEchoId = 0;//get_random_u16();
  icmpEchoSeq = 295;
...
bool HostOsScan::processResp(HostOsScanStats *hss, struct ip *ip, unsigned int len, struct timeval *rcvdtime) {
  ...
  if (ip->ip_p == IPPROTO_TCP) {
    if (len < 20) // || ntohs(ip->ip_len) != (len + 4 * ip->ip_hl))
      return false;
	...
	if (ntohs(ip->ip_len) == len + 4 * ip->ip_hl)
    ...
    if (icmp->icmp_type == 3 && icmp->icmp_code == 3) {
      ...
      if (len < 28 || (len + 8 + 4 * ip->ip_hl) != ntohs(ip->ip_len))
...
int HostOsScan::send_icmp_echo_probe(HostOsScanStats *hss,
  ...
  for (decoy = 0; decoy < o.numdecoys; decoy++) {
    packet = build_icmp_raw(&((struct sockaddr_in *)&o.decoys[decoy])->sin_addr, hss->target->v4hostip(),
                            o.ttl, /*get_random_u16()*/0, tos, df, NULL, 0, seq, id,
