...
CXX = /home/default/third_party/llvm-build/Release+Asserts/bin/clang++
CCOPT = 
DBGFLAGS = 
STRIP = /usr/bin/strip
LIBPCAPDIR = libpcap
...
# DEFS += -DMTRACE=1
CXXFLAGS = -ggdb -O0 -fsanitize=memory -fsanitize-coverage=trace-pc-guard,trace-cmp,trace-div,trace-gep -DHAVE_GETADDRINFO=1 -Wall -fno-strict-aliasing $(DBGFLAGS) $(CCOPT)
CPPFLAGS = -I$(top_srcdir)/liblinear -I$(top_srcdir)/liblua -I$(top_srcdir)/libdnet-stripped/include -I$(top_srcdir)/libssh2/include  -I$(top_srcdir)/libpcap -I$(top_srcdir)/nbase -I$(top_srcdir)/nsock/include $(DEFS)
CFLAGS = -ggdb -O0 -fsanitize=memory -fsanitize-coverage=trace-pc-guard,trace-cmp,trace-div,trace-gep -DHAVE_GETADDRINFO=1 -Wall $(DBGFLAGS) $(CCOPT)
STATIC =
LDFLAGS = -Wl,-E -fsanitize=memory -Lnbase -Lnsock/src/ $(DBGFLAGS) $(STATIC)
LIBS =  -lnsock -lnbase -lpcre $(LIBPCAPDIR)/libpcap.a $(LIBSSH2_LIBS) $(OPENSSL_LIBS) $(ZLIB_LIBS) libnetutil/libnetutil.a $(top_srcdir)/libdnet-stripped/src/.libs/libdnet.a $(top_srcdir)/liblua/liblua.a $(top_srcdir)/liblinear/liblinear.a 
...
$(TARGET): build-netutil build-liblinear build-pcap build-libssh2   build-nsock build-nbase build-dnet build-lua \
	$(OBJS) main.o
	@echo Compiling nmap
	rm -f $@
...
clean: clean-lua clean-liblinear clean-pcap  clean-dnet clean-libssh2 \
...
distclean: distclean-lua distclean-liblinear distclean-pcap distclean-libssh2\
